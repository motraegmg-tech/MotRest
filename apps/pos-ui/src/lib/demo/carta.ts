/**
 * Contenido de DEMOSTRACIÓN: la carta de ejemplo con la que se enseña MotRest.
 *
 * Vive en su propio módulo por una razón concreta de empaquetado: mientras
 * estaba junto al catálogo vivo -que importan siete pantallas- el empaquetador
 * conservaba estos datos aunque el único camino que los usa estuviera apagado,
 * y la carta inventada viajaba al instalador del restaurante. Separado, nadie
 * alcanzable lo importa en producción y desaparece entero.
 *
 * Aquí NO se agrega nada nuevo: si haces crecer esto, crece la demostración,
 * no el producto.
 */
...
import { impuestos } from "../catalogo";
import { insumosDemo } from "../insumos";

export function menuDemostracion(): MenuLocal {
  ...
  const gruposModificadores: GrupoModificadores[] = [
    {
      id: "gm-termino",
      nombre: "Término",
      seleccion: "uno",
   ...
   */
  /**
   * Carta de la demostración.
   *
   * Va dentro de una FUNCIÓN, no de una constante, y eso no es estilo: una
   * constante de módulo sobrevive al empaquetado aunque el único camino que la usa
   * esté apagado —el empaquetador no puede probar que construirla no tenga
   * efectos—, y la carta inventada terminaba viajando al instalador del
   * restaurante. Una función que nadie llama se elimina entera. Es la misma
   * lección que con los usuarios de demostración; verificado sobre el bundle.
   */
  return {
    version: 1,
    updated_at: 0,
...
/**
 * Con qué carta abre un restaurante REAL el primer día: la suya, o ninguna.
 *
 * Sin productos, sin categorías y sin insumos —esos son de cada local—, pero
 * CON los perfiles de impuesto y las estaciones de cocina: eso no es contenido
 * inventado, es la estructura que cualquier restaurante necesita para que el
 * primer producto que capture tenga dónde colgarse.
 */
